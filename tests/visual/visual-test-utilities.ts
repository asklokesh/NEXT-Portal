 ...
 * 
 * Shared utilities and helpers for visual regression testing
 * to ensure consistent screenshot capture and comparison.
 */
...
export class VisualTestUtils {
  ...
  /**
   * Hide elements that frequently change and cause visual test flakiness
   */
  async hideVolatileElements(): Promise<void> {
    await this.page.addStyleTag({
      content: `
        /* Hide timestamp and time-based elements */
        ...
        /* Hide loading indicators and animations */
        .animate-spin,
        .animate-pulse,
        .animate-bounce,
        .animate-ping,
        ...
        /* Hide version numbers that might change */
        [data-testid="build-version"],
        [data-testid="app-version"],
        .build-info,
        
        /* Hide tooltips that might appear */
        .tooltip,
        [role="tooltip"],
  ...
  /**
   * Wait for page to be in a stable state for screenshot
   */
   ...
   */
  async mockAuthState(role: 'admin' | 'user' | 'guest' = 'user'): Promise<void> {
    const authTokens = {
      admin: 'mock-admin-jwt-token-12345',
      user: 'mock-user-jwt-token-67890',
      guest: null,
    };
    ...
    await this.page.evaluate(
      ...
      { token: authTokens[role], user: userData[role] }
    );
  }

  /**
   * Set consistent viewport and browser settings
   */
  async setupConsistentEnvironment(): Promise<void> {
    // Set timezone to UTC for consistent time display
    await this.page.emulateTimezone('UTC');
    ...
    await this.page.evaluate(() => {
      // Mock date to fixed time for consistent screenshots
      ...
      if (typeof performance !== 'undefined' && performance.now) {
        let startTime = 0;
        performance.now = () => startTime += 16.67; // ~60fps
      }
    });
  }
  ...
  async screenshot(
    name: string,
    element?: Locator,
    options: ScreenshotOptions & VisualTestOptions = {}
  ): Promise<void> {
    const {
    ...
    if (customWait > 0) {
      await this.page.waitForTimeout(customWait);
    }

    // Execute custom pre-screenshot logic
    if (beforeScreenshot) {
  ...
  /**
   * Compare two screenshots manually
   */
  async compareScreenshots(
    ...
    try {
      ...
      // Simple buffer comparison (in real implementation, you'd use image comparison library)
      ...
      const diffPercentage = differences / baselineBuffer.length;
  ...
  /**
   * Create test data fixtures for visual tests
   */
  async setupVisualTestData(): Promise<void> {
    // Mock API responses with consistent data
    await this.page.route('**/api/plugins**', (route) => {
      const mockPlugins = [
        {
          id: 'visual-test-plugin-1',
          name: 'Visual Test Plugin One',
          version: '1.0.0',
          description: 'A plugin designed specifically for visual testing with consistent data.',
          author: 'Test Author',
          category: 'Testing',
          tags: ['visual', 'test', 'stable'],
          icon: 'test-tube',
          downloadCount: 1000,
          rating: 4.5,
          reviews: 20,
        ...
        {
          id: 'visual-test-plugin-2',
          name: 'Visual Test Plugin Two',
          version: '2.1.0',
          description: 'Another plugin for visual regression testing with different properties.',
   ...
   */
  async generateBaselines(testName: string): Promise<void> {
    const baselineDir = path.resolve('./tests/visual/screenshots/baseline');
    await fs.mkdir(baselineDir, { recursive: true });
    
    console.log(`Generating baseline screenshots for: ${testName}`);
    // Implementation would go here to systematically capture baseline images
  }

  /**
   * Validate screenshot differences
   */
  async validateScreenshotDifferences(testResults: any): Promise<void> {
  ...
  /**
   * Clean up old screenshots and test artifacts
   */
...
/**
 * Common screenshot options for different scenarios
 */
export const SCREENSHOT_OPTIONS = {
  ...
  mobile: {
    ...
    animations: 'disabled' as const,
    caret: 'hide' as const,
    threshold: 0.25, // Higher threshold for mobile due to rendering differences
  },
};

/**
 * Test data generators for consistent visual tests
 */
export const VISUAL_TEST_DATA = {
  plugins: [
    {
      id: 'visual-plugin-1',
      name: 'Consistent Test Plugin',
      version: '1.0.0',
      description: 'A plugin with stable visual properties for testing',
      downloadCount: 1234,
      rating: 4.5,
      reviews: 67,
    },
    {
      ...
      version: '2.1.0',
      description: 'Second plugin for grid layout testing',
      downloadCount: 5678,
      rating: 4.8,
      reviews: 123,
    },
