...
pluginsCommand
  ...
  .option('--installed-only', 'show only installed plugins')
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const client = createBackstageClient(config);
      ...
      const response = await client.plugins.list(params);
      spinner.stop();
      
      let plugins = response.items;
      
      // Filter for installed only
      if (options.installedOnly) {
        plugins = plugins.filter(p => p.status === 'installed');
      }
      
      if (plugins.length === 0) {
        logger.info('No plugins found matching criteria');
        return;
      }
      ...
      if (outputFormat === 'table') {
        const tableData = [
          ['Name', 'Version', 'Status', 'Category', 'Author', 'Description']
        ];
        ...
        plugins.forEach(plugin => {
          ...
          tableData.push([
            ...
            plugin.category || '-',
            plugin.author || '-',
            plugin.description?.substring(0, 50) + (plugin.description?.length > 50 ? '...' : '') || '-'
          ]);
        });
        ...
        console.log(table(tableData, {
          header: {
            alignment: 'center',
            content: `Plugins (${plugins.length} of ${response.total})`
          }
        }));
        
        if (response.hasMore) {
          logger.info(`Showing ${plugins.length} of ${response.total} plugins. Use --limit to show more.`);
        }
      } else {
        console.log(formatOutput(plugins, outputFormat));
      }
...
pluginsCommand
  ...
  .action(async (pluginId, options) => {
    ...
    try {
      ...
      const plugin = await client.plugins.get(pluginId);
      spinner.stop();
      
      const outputFormat = options.format || config.format;
      
      if (outputFormat === 'table') {
        console.log(chalk.bold.blue(`Plugin: ${plugin.name}`));
        console.log('');
        ...
        const details = [
          ...
          ['Install Date', plugin.installDate ? new Date(plugin.installDate).toLocaleDateString() : '-']
        ];
        
        if (plugin.dependencies && plugin.dependencies.length > 0) {
          details.push(['Dependencies', plugin.dependencies.join(', ')]);
        }
        ...
        if (plugin.config && Object.keys(plugin.config).length > 0) {
          console.log('\nConfiguration:');
          console.log(formatOutput(plugin.config, 'yaml'));
        }
        
        if (plugin.metadata && Object.keys(plugin.metadata).length > 0) {
          console.log('\nMetadata:');
          console.log(formatOutput(plugin.metadata, 'yaml'));
        }
      } else {
        console.log(formatOutput(plugin, outputFormat));
      }
      
    } catch (error) {
      spinner.stop();
      logger.error(`Failed to get plugin ${pluginId}:`, error.message);
      process.exit(1);
    }
  });
...
pluginsCommand
  ...
  .option('-f, --config-file <file>', 'configuration file path')
  ...
  .action(async (pluginId, options) => {
    try {
      ...
      let plugin;
      try {
        plugin = await client.plugins.get(pluginId);
      } catch (error) {
      ...
      if (plugin.status === 'installed') {
        logger.warn(`Plugin ${pluginId} is already installed`);
        if (!options.yes) {
      ...
      if (options.config) {
        try {
          pluginConfig = JSON.parse(options.config);
        } catch (error) {
          logger.error('Invalid JSON configuration:', options.config);
          process.exit(1);
        }
        ...
        try {
          ...
          pluginConfig = options.configFile.endsWith('.json')
            ? JSON.parse(configContent)
            : yaml.load(configContent);
        } catch (error) {
          logger.error(`Failed to read configuration file ${options.configFile}:`, error.message);
          process.exit(1);
        }
      }
      ...
      if (!options.yes) {
        console.log(chalk.bold(`Installing plugin: ${plugin.name}`));
        console.log(`Version: ${options.version || plugin.version}`);
        console.log(`Category: ${plugin.category || 'Unknown'}`);
      ...
      const result = await client.plugins.install(installRequest);
      spinner.stop();
      
      if (result.success) {
        logger.success(`✓ Plugin ${pluginId} installed successfully`);
        if (result.installationId) {
          logger.info(`Installation ID: ${result.installationId}`);
        }
      } else {
...
pluginsCommand
  ...
  .action(async (pluginId, options) => {
    try {
      ...
      if (plugin.status !== 'installed') {
        logger.error(`Plugin ${pluginId} is not installed`);
        process.exit(1);
      }
      
      // Confirm uninstallation
      if (!options.yes) {
        console.log(chalk.bold.red(`Uninstalling plugin: ${plugin.name}`));
        console.log(`This will remove the plugin and its configuration.`);
...
pluginsCommand
  ...
  .action(async (query, options) => {
    ...
    try {
      ...
      const response = await client.plugins.search(query, options.category, {
        limit: parseInt(options.limit)
      });
      ...
      if (response.items.length === 0) {
        logger.info(`No plugins found for query: ${query}`);
        return;
      }
      ...
      if (outputFormat === 'table') {
        ...
        response.items.forEach(plugin => {
          ...
          tableData.push([
            ...
            status,
            plugin.category || '-',
            plugin.description?.substring(0, 60) + (plugin.description?.length > 60 ? '...' : '') || '-'
          ]);
        });
        ...
        console.log(table(tableData, {
          header: {
            alignment: 'center',
            content: `Search Results for "${query}" (${response.items.length} found)`
          }
        }));
      } else {
