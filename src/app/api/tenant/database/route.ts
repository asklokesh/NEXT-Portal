...
import {
  createTenantServiceFromRequest,
  createSystemService,
} from '@/lib/database/TenantPrismaService';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check permissions
    if (
      !tenantService.hasPermission('tenant:read') &&
      !tenantService.hasPermission('analytics:view')
    ) {
      return NextResponse.json(
        {
          success: false,
  ...
  try {
    ...
    switch (operation) {
      ...
      case 'get_audit_logs':
        ...
        result = await tenantService.getAuditLogs(parameters.filters || {}, parameters.limit || 50);
        break;
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { operation, systemKey, parameters: _parameters = {} } = validation.sanitized;
    ...
    switch (operation) {
      ...
      case 'refresh_tenant_metrics':
        ...
        try {
          await systemService.getDatabase().executeRaw(Prisma.sql`SELECT refresh_tenant_metrics()`);
          result = {
            success: true,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const cleanupResults = await systemService
      .getDatabase()
      .executeRaw(
    ...
    await systemService.createAuditLog('tenant_data_deletion', 'system_cleanup', tenantId, {
      tenantId,
      cleanupResults,
