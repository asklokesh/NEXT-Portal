...
export class DashboardWebSocketService extends EventEmitter {
    ...
    private setupEventHandlers(): void {
        ...
        this.ws.onerror = (error) => {
            // Suppress connection refused errors in local/demo environment
            if (this.url.includes('localhost')) {
                console.warn('Demo Mode: WebSocket connection unavailable (using HTTP fallback)');
                return;
            }
    ...
    private scheduleReconnect(): void {
        if (this.url.includes('localhost')) {
             console.log('Demo Mode: Skipping WebSocket reconnect');
             return;
        }
