import type { GraphNode, GraphEdge, DependencyGraph, HealthCheck, Alert } from './types';
...
export class GraphHealthMonitor {
  ...
  private async simulateHealthCheck(
    ...
    const checks = [
      {
        name: 'http_endpoint',
        status: (metrics.responseTime < this.config.metrics.responseTimeThreshold
          ? 'pass'
          : 'fail') as 'pass' | 'fail' | 'warn',
        message: `Response time: ${metrics.responseTime.toFixed(0)}ms`,
        duration: metrics.responseTime,
      },
      {
        name: 'error_rate',
        status: (metrics.errorRate < this.config.metrics.errorRateThreshold ? 'pass' : 'fail') as
          | 'pass'
          | 'fail'
          | 'warn',
      ...
      {
        name: 'resource_usage',
        status: (metrics.cpuUsage < 80 && metrics.memoryUsage < 85 ? 'pass' : 'warn') as
          | 'pass'
          | 'fail'
          | 'warn',
  ...
  private calculateHealthScore(
    ...
    if (metrics.responseTime > this.config.metrics.responseTimeThreshold) {
      score -= Math.min(
        20,
        (metrics.responseTime - this.config.metrics.responseTimeThreshold) / 100,
      );
    }
  ...
  private analyzeGraphHealth(graph: DependencyGraph): void {
    ...
    const overallHealth =
      totalNodes > 0
        ? (healthCounts.healthy * 100 + healthCounts.degraded * 70 + healthCounts.unhealthy * 30) /
          totalNodes
        : 100;
  ...
  private generateAlert(
    alertData: Omit<HealthAlert, 'id' | 'timestamp' | 'acknowledged' | 'resolvedAt'>,
  ): void {
