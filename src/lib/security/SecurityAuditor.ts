...
export class SecurityAuditor {
  ...
  async runComprehensiveAudit(): Promise<SecurityAuditResult> {
    ...
    await this.checkAPISecurityVulnerabilities();
    await this.checkDatabaseSecurity();
    await this.checkInfrastructureSecurity();
    await this.checkComplianceControls();
    ...
    console.log(`✅ Security audit completed with ${result.vulnerabilities.length} findings`);
  ...
  private async checkDependencyVulnerabilities(): Promise<void> {
    try {
        ...
        Object.entries(auditData.vulnerabilities).forEach(([pkg, vuln]: [string, any]) => {
          if (vuln.severity === 'high' || vuln.severity === 'critical') {
            this.addVulnerability({
              ...
              impact: `Dependency vulnerability could lead to ${vuln.severity} security issues`,
              remediation: `Update ${pkg} to a secure version or find an alternative`,
              affected: [pkg],
              confidence: 'high'
            });
          }
      ...
      console.warn('Failed to check dependency vulnerabilities:', error);
    }
  }
  ...
  private async checkOWASPTop10(): Promise<void> {
    ...
    // Already covered in dependency check
    
    // A07 - Authentication Failures
    await this.checkAuthenticationFailures();
    
    // A08 - Software Integrity Failures
    await this.checkSoftwareIntegrity();
  ...
  private async checkAccessControl(): Promise<void> {
    ...
    unauthorizedEndpoints.forEach(endpoint => {
      this.addVulnerability({
        id: `access-control-${endpoint.replace(/[^a-z0-9]/gi, '-')}`,
        severity: 'high',
        category: 'Access Control',
        title: 'Potential Authorization Bypass',
        description: `Endpoint ${endpoint} may lack proper authorization checks`,
        cwe: 'CWE-284',
  ...
  private async checkSecurityMisconfiguration(): Promise<void> {
    ...
    // This would normally check actual responses
    this.addVulnerability({
      id: 'config-headers',
      severity: 'low',
  ...
  private async checkSoftwareIntegrity(): Promise<void> {
    // Check for software integrity controls
    this.addVulnerability({
   ...
   */
  private async checkAPISecurityVulnerabilities(): Promise<void> {
    // Rate limiting check
    this.addVulnerability({
   ...
   */
  private async checkComplianceControls(): Promise<void> {
    // GDPR compliance
    this.addVulnerability({
   ...
   */
  private async checkSecretExposure(): Promise<void> {
   ...
   */
  private async checkCryptographicImplementation(): Promise<void> {
    this.addVulnerability({
  ...
  private generateAuditResult(): SecurityAuditResult {
    const metrics = {
      ...
      remediationScore: this.calculateRemediationScore()
    };
    ...
    return {
      ...
      complianceStatus: {
        soc2: metrics.criticalCount === 0 && metrics.highCount < 5,
        gdpr: metrics.criticalCount === 0 && this.hasRequiredGDPRControls(),
        pci: false, // Requires specific PCI compliance audit
        iso27001: metrics.criticalCount === 0 && metrics.highCount < 3
      },
      metrics,
  ...
  private calculateRemediationScore(): number {
    if (this.vulnerabilities.length === 0) return 100;
    
    const weights = { critical: 40, high: 20, medium: 10, low: 5 };
    const totalWeight = this.vulnerabilities.reduce((sum, v) => sum + (weights[v.severity] || 0), 0);
    const maxPossible = this.vulnerabilities.length * weights.critical;
    
    return Math.max(0, Math.round((1 - totalWeight / maxPossible) * 100));
  }

  private calculateOverallRisk(metrics: any): 'critical' | 'high' | 'medium' | 'low' {
    if (metrics.criticalCount > 0) return 'critical';
    if (metrics.highCount > 5) return 'high';
    if (metrics.highCount > 0 || metrics.mediumCount > 10) return 'medium';
    return 'low';
  }

  private hasRequiredGDPRControls(): boolean {
    // Check if GDPR-specific vulnerabilities are addressed
    const gdprVulns = this.vulnerabilities.filter(v => 
      v.category === 'Compliance' && v.id.includes('gdpr')
    );
    return gdprVulns.every(v => v.severity !== 'high' && v.severity !== 'critical');
  }
