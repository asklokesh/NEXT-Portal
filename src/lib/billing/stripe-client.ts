...
const DEV_STRIPE_PLACEHOLDER = 'sk_test_placeholder_not_configured';
...
function createStripeClient(): Stripe {
  ...
  if (!secretKey || secretKey === DEV_STRIPE_PLACEHOLDER) {
    if (!isDev) {
      throw new Error(
        'STRIPE_SECRET_KEY is not configured. Set a valid Stripe secret key before starting the server.',
      );
    }
    console.warn(
      '[stripe-client] WARNING: STRIPE_SECRET_KEY is missing or using the placeholder value. ' +
        'Billing calls will fail. Set STRIPE_SECRET_KEY in your environment.',
    );
  }

  return new Stripe(secretKey || DEV_STRIPE_PLACEHOLDER, {
...
/** @deprecated Prefer getStripeClient(); kept for existing imports */
...
export const stripeHelpers = {
  ...
  handleStripeError: (error: any): never => {
    if (error instanceof Stripe.errors.StripeError) {
      throw new StripeError(`Stripe error: ${error.message}`, error, error.code);
    }
