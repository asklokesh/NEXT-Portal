...
class PrismaClientWrapper {
    ...
    return this.executeQuery(
      async (client) => {
        if (typeof command === 'string') {
          return await client.$executeRawUnsafe(command);
        } else {
  ...
  async batchTransaction(operations: Array<() => Promise<any>>): Promise<any[]> {
    return this.executeQuery(
      async (client) => {
        return await client.$transaction(
  ...
  async healthCheck(): Promise<boolean> {
    try {
      await this.executeQuery(
        async (client) => await client.$queryRaw`SELECT 1`,
        { readonly: true }
      );
      return true;
    } catch (error) {
