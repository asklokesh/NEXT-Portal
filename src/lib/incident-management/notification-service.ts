...
export class NotificationService {
      ...
      try {
        switch (contactMethod.type) {
          ...
          case 'phone':
            await this.sendSMS(
              contactMethod.value,
              this.formatUserNotification(incident, reason, user, true),
            );
            break;
          case 'pagerduty':
  ...
  private renderTemplate(template: string, context: any): string {
    ...
    if (context.incident) {
      ...
      result = result.replace(
        /\{\{incident\.duration\}\}/g,
        this.formatDuration(incident.createdAt),
      );
    }
  ...
  private formatUserNotification(
    incident: Incident,
    reason: string,
    user: User,
    short = false,
...
Alert Details: https://portal.example.com/alerts/${alert.id}
      ...
      switch (primaryContact.type) {
        ...
        case 'phone':
          await this.sendSMS(
            ...
            `ALERT: ${alert.title} - ${alert.severity.toUpperCase()}`,
          );
          break;
      }
    }
  ...
  private async getTeamMembers(teamId: string): Promise<User[]> {
    ...
    return [await this.getUserById('user1'), await this.getUserById('user2')].filter(
...
[View Incident](https://portal.example.com/incidents/{{incident.id}})
  ...
  public getNotificationHistory(incidentId?: string, limit = 100): NotificationHistory[] {
    ...
    return history.sort((a, b) => b.sentAt.getTime() - a.sentAt.getTime()).slice(0, limit);
  }
}
