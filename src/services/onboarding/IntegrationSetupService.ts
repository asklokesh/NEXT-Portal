...
export class IntegrationSetupService {
  ...
  async setupIntegrations(
    ...
    if (integrations.github) {
      const result = await this.setupGitHub(organizationId, integrations.github);
      results.push(result);
    }

    // Setup GitLab
    if (integrations.gitlab) {
      const result = await this.setupGitLab(organizationId, integrations.gitlab);
      results.push(result);
    }

    // Setup Jenkins
    if (integrations.jenkins) {
      const result = await this.setupJenkins(organizationId, integrations.jenkins);
      results.push(result);
    }

    // Setup AWS
    if (integrations.aws) {
      const result = await this.setupAWS(organizationId, integrations.aws);
      results.push(result);
    }

    // Setup Slack
    if (integrations.slack) {
      const result = await this.setupSlack(organizationId, integrations.slack);
      results.push(result);
    }
    ...
    try {
      ...
      try {
        await octokit.orgs.createWebhook({
          ...
          events: ['push', 'pull_request', 'issues', 'deployment', 'workflow_run'],
        });
      } catch (error) {
   ...
   */
  private async sendSlackWelcomeMessage(token: string, channel: string): Promise<void> {
    try {
      await axios.post(
  ...
  private generateWebhookSecret(): string {
    return Buffer.from(Math.random().toString(36).substring(2) + Date.now().toString(36)).toString(
      'base64',
    );
  }
}
