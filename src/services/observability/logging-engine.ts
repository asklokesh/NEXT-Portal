  ...
  private applyFilters(logs: LogEntry[], filter: LogFilter): LogEntry[] {
    return logs.filter((log) => {
      ...
      if (filter.timeRange) {
        ...
        if (
          logTime < filter.timeRange.start.getTime() ||
          logTime > filter.timeRange.end.getTime()
        ) {
          return false;
        }
      }
...
class LogAnalyzer {
  ...
  private detectAnomalies(logs: LogEntry[]) {
    ...
    const avgErrorRate =
      Object.values(hourlyErrorCounts).reduce((sum, count) => sum + count, 0) /
      Object.keys(hourlyErrorCounts).length;

    for (const [hour, count] of Object.entries(hourlyErrorCounts)) {
      if (count > avgErrorRate * 3) {
        // 3x normal rate
        anomalies.push({
          type: 'error_spike',
          ...
          relatedLogs: logs.filter(
            (l) => l.level === 'error' && l.timestamp.getHours().toString() === hour,
          ),
        });
      }
    }
