...
export class MetricsCollector extends EventEmitter {
  ...
  recordUserMetric(action: string, userId?: string, labels: Record<string, string> = {}): void {
    ...
    if (userId) {
      this.userMetrics.set(
        `active_users_${action}`,
        (this.userMetrics.get(`active_users_${action}`) || 0) + 1,
      );
    }
  }
  ...
  private async collectSystemMetrics(): Promise<void> {
    try {
      ...
      const processInternals = process as NodeJS.Process & {
        _getActiveHandles?: () => unknown[];
        _getActiveRequests?: () => unknown[];
      };
      this.setGauge('nodejs_active_handles', processInternals._getActiveHandles?.()?.length ?? 0);
      this.setGauge('nodejs_active_requests', processInternals._getActiveRequests?.()?.length ?? 0);
