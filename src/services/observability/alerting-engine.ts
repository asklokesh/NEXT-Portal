...
export class AlertingEngine extends EventEmitter {
    ...
    if (filter) {
      ...
      if (filter.timeRange) {
        alerts = alerts.filter(
          (a) => a.startsAt >= filter.timeRange!.start && a.startsAt <= filter.timeRange!.end,
        );
      }
    }
  ...
  getMetrics(): AlertingMetrics {
    ...
    const alertsByService = alerts.reduce(
      (acc, alert) => {
    ...
    const averageResolutionTime =
      resolvedAlerts.length > 0
        ? resolvedAlerts.reduce((sum, a) => sum + (a.endsAt!.getTime() - a.startsAt.getTime()), 0) /
    ...
    // Notification metrics
    const notificationsSent = Array.from(this.notificationCount.values()).reduce(
      (sum, count) => sum + count,
      0,
    );
    ...
    const previousPeriodAlerts = alerts.filter(
      (a) => a.startsAt >= last48Hours && a.startsAt < last24Hours,
    );

    const percentageChange =
      previousPeriodAlerts.length > 0
  ...
  private async evaluateRulesForMetric(metric: any): Promise<void> {
    const relevantRules = Array.from(this.rules.values()).filter(
      (rule) => rule.enabled && this.ruleMatchesMetric(rule, metric),
    );
  ...
  private findExistingAlert(rule: AlertingRule): Alert | undefined {
    return Array.from(this.alerts.values()).find(
      (alert) => alert.rule.id === rule.id && alert.status === 'firing',
    );
  }

  private ruleMatchesMetric(rule: AlertingRule, metric: any): boolean {
    return (
      rule.expression === metric.name ||
  ...
  private getChannelsForGroup(group: AlertGroup): NotificationChannel[] {
    return Array.from(this.channels.values()).filter((channel) =>
      this.channelMatchesGroup(channel, group),
    );
  }
  ...
  private formatNotificationMessage(channel: NotificationChannel, group: AlertGroup): any {
    ...
    const summary =
      alerts.length === 1 ? alerts[0].annotations.summary : `${alerts.length} alerts firing`;
    ...
    try {
      ...
      return {
        ...
        details:
          status === 'healthy'
            ? undefined
            : `${firingAlerts.length} firing alerts (${criticalAlerts.length} critical)`,
      };
    } catch (error) {
      return {
        status: 'unhealthy',
