...
export class TracingManager extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // Get tracer instance
      this.tracer = api.trace.getTracer(this.config.serviceName, this.config.version);
  ...
  getTraces(
    ...
    let filteredSpans = allSpans.filter((span) => {
      ...
      if (filter.timeRange) {
        ...
        if (
          spanTime < filter.timeRange.start.getTime() ||
          spanTime > filter.timeRange.end.getTime()
        ) {
          return false;
        }
      }
