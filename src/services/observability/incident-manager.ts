...
export class IncidentManager extends EventEmitter {
  ...
  private remediationActions: Map<
    string,
    {
      incidentId: string;
      action: string;
   ...
   */
  createIncident(
    incident: Omit<Incident, 'id' | 'detectedAt' | 'status' | 'escalationLevel' | 'timeline'>,
  ): Incident {
    ...
    if (filter) {
      ...
      if (filter.timeRange) {
        incidents = incidents.filter(
          (i) => i.detectedAt >= filter.timeRange!.start && i.detectedAt <= filter.timeRange!.end,
        );
      }
    }
  ...
  getMetrics(): IncidentMetrics {
    ...
    const byService = incidents.reduce(
      (acc, incident) => {
    ...
    const mtta =
      acknowledgedIncidents.length > 0
        ? acknowledgedIncidents.reduce(
            (sum, i) => sum + (i.acknowledgedAt!.getTime() - i.detectedAt.getTime()),
            0,
          ) /
    ...
    const escalationRate =
      incidents.length > 0 ? (escalatedIncidents.length / incidents.length) * 100 : 0;
    ...
    const previousPeriodIncidents = incidents.filter(
      (i) => i.detectedAt >= last60Days && i.detectedAt < last30Days,
    );

    const percentageChange =
      previousPeriodIncidents.length > 0
        ? ((recentIncidents.length - previousPeriodIncidents.length) /
  ...
  async getStatistics(): Promise<{ active: number; resolved24h: number }> {
    ...
    const resolved24h = Array.from(this.incidents.values()).filter(
      (i) => i.resolvedAt && i.resolvedAt >= twentyFourHoursAgo,
    );
  ...
  private async processEscalations(): Promise<void> {
    const openIncidents = Array.from(this.incidents.values()).filter((i) =>
  ...
  private getEscalationPolicyForIncident(incident: Incident): EscalationPolicy | undefined {
    ...
    return Array.from(this.escalationPolicies.values()).find(
      (policy) => policy.service === incident.service || policy.service === '*',
    );
  }
  ...
  private async sendEscalationNotifications(
    ...
    console.log(
      `📢 Escalating incident ${incident.id} to ${level.target} via ${level.methods.join(', ')}`,
    );
  }
  ...
  private async findRelatedIncident(alert: any): Promise<Incident | undefined> {
    ...
    return openIncidents.find(
      (incident) => incident.service === alert.service || incident.component === alert.component,
    );
  }
    ...
    try {
      ...
      return {
        ...
        details:
          status === 'healthy'
            ? undefined
            : `${activeIncidents.length} active incidents (${criticalIncidents.length} critical)`,
      };
    } catch (error) {
      return {
        status: 'unhealthy',
